...
Map<String, dynamic>? convertNativeToDart_Dictionary(object) {
  ...
  for (final key in keys) {
    dict[JS('String', '#', key)] = _convertNativeToDart_Value(
      JS('var', '#[#]', object, key),
    );
  }
  return dict;
...
convertDartToNative_PrepareForStructuredClone(value) =>
    _StructuredCloneDart2Js().convertDartToNative_PrepareForStructuredClone(
      value,
    );
...
;
const annotation_Creates_SerializedScriptValue = Creates(
  _serializedScriptValue,
);
const annotation_Returns_SerializedScriptValue = Returns(
  _serializedScriptValue,
);
