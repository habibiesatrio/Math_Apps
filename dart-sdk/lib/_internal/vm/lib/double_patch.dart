...
class double {
  ...
  static double? _tryParseDouble(String str, int start, int end) {
    ...
    if (firstChar == N) {
      if (end == start + 3 &&
          str.codeUnitAt(start + 1) == a &&
          str.codeUnitAt(start + 2) == N) {
        return double.nan;
      }
      return null;
    }

    int firstDigit = firstChar ^ ZERO;
    if (firstDigit <= 9) {
      start++;
    ...
    for (int i = start; i < end; i++) {
      ...
      int digit = c ^ ZERO; // '0'-'9' characters are now 0-9 integers.
      if (digit <= 9) {
        ...
        if (doubleValue >= maxExactDouble) return null;
        exponent += exponentDelta;
        digitsSeen = true;
      } else if (c == DOT && exponentDelta == 0) {
        exponentDelta = -1;
      } else if ((c | 0x20) == e) {
        i++;
