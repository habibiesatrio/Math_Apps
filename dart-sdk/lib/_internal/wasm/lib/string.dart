...
abstract final class StringBase extends WasmStringBase
  ...
  String replaceRange(int start, int? end, String replacement) {
    ...
    if (replacement.isNotEmpty) slices.add(replacement);
...
final class OneByteString extends StringBase {
  ...
  List<String> _splitWithCharCode(int charCode) {
    ...
    int start = 0;
    for (i = 0; i < length; ++i) {
      if (_codeUnitAtUnchecked(i) == charCode) {
  ...
  int indexOf(Pattern pattern, [int start = 0]) {
    ...
    if (pattern is String && pattern.length == 1 && start >= 0 && start < len) {
      ...
      for (int i = start; i < len; i++) {
        if (_codeUnitAtUnchecked(i) == patternCu0) {
          return i;
        }
      }
      return -1;
  ...
  String padLeft(int width, [String padding = ' ']) {
    ...
    for (int i = 0; i < length; i++) {
      result._setAt(index++, _codeUnitAtUnchecked(i));
    }
    return result;
  }
