...
class _AsyncCompleter<T> extends _Completer<T> {
  ...
  void complete([FutureOr<T>? value]) {
    if (!future._mayComplete) throw StateError("Future already completed");
    future._asyncComplete(value ?? value as dynamic);
  }
...
class _SyncCompleter<T> extends _Completer<T> {
  ...
  void complete([FutureOr<T>? value]) {
    if (!future._mayComplete) throw StateError("Future already completed");
    future._complete(value ?? value as dynamic);
  }
...
class _Future<T> implements Future<T> {
  ...
  Future<T> timeout(Duration timeLimit, {FutureOr<T> Function()? onTimeout}) {
    ...
    if (onTimeout == null) {
      ...
      timer = Timer(timeLimit, () {
        try {
          future._complete(zone.run(onTimeoutHandler));
        } catch (e, s) {
          future._completeErrorObject(AsyncError(e, s));
        }
      });
    }
    then(
