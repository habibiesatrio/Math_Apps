...
abstract interface class Future<T> {
  ...
  factory Future.value([FutureOr<T>? value]) {
    return _Future<T>.immediate(value ?? value as T);
  }
  ...
  /// });
  /// ```
  factory Future.delayed(Duration duration, [FutureOr<T> Function()? computation]) {
  ...
  static Future<List<T>> wait<T>(
    ...
    final _Future<List<T>> future = _Future<List<T>>();
    ...
    void handleError(Object theError, StackTrace theStackTrace) {
      ...
      if (valueList != null) {
        ...
        if (remainingResults == 0 || eagerError) {
          future._completeError(theError, theStackTrace);
        }
      } else {
        // Not the first error.
        if (remainingResults == 0 && !eagerError) {
    ...
    try {
      ...
      if (remaining == 0) {
        ...
        return future.._completeWithValue(<T>[]);
      }
      ...
      if (remaining == 0 || eagerError) {
        ...
        // the result.
        return future.._asyncCompleteErrorObject(_interceptCaughtError(e, s));
      } else {
