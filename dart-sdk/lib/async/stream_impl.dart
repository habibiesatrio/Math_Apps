...
class _BufferingStreamSubscription<T>
  ...
  void _add(T data) {
    ...
    if (_canFire) {
      ...
      _addPending(_DelayedData<T>(data));
    }
  }
...
abstract class _StreamImpl<T> extends Stream<T> {
  ...
  StreamSubscription<T> _createSubscription(
    ...
    return _BufferingStreamSubscription<T>(
      onData,
      onError,
      onDone,
      cancelOnError,
...
typedef _BroadcastCallback<T> = void Function(StreamSubscription<T> subscription);
...
class _AsBroadcastStream<T> extends Stream<T> {
  ...
  _AsBroadcastStream(
    ...
    void Function(StreamSubscription<T> subscription)? onCancelHandler,
      ...
      _zone = Zone.current {
    _controller = _AsBroadcastStreamController<T>(_onListen, _onCancel);
  }
  ...
  void _onCancel() {
    ...
    if (cancelHandler != null) {
      _zone.runUnary(cancelHandler, _BroadcastSubscriptionWrapper<T>(this));
    }
    if (shutdown) {
  ...
  void _onListen() {
    ...
    if (listenHandler != null) {
      _zone.runUnary(listenHandler, _BroadcastSubscriptionWrapper<T>(this));
    }
  }
...
class _StreamIterator<T> implements StreamIterator<T> {
  ...
  Future<bool> moveNext() {
    ...
    if (subscription != null) {
      if (_hasValue) {
        var future = _Future<bool>();
        _stateData = future;
        _hasValue = false;
        subscription.resume();
        return future;
      }
      throw StateError("Already waiting for next.");
    }
