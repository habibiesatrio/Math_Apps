...
final class FileSystemEntityType {
  ...
  static const unixDomainSock = FileSystemEntityType._internal(3);

  static const pipe = FileSystemEntityType._internal(4);

  static const notFound = FileSystemEntityType._internal(5);
...
interface class FileStat {
  ...
  static final _notFound = FileStat._internal(
    _epoch,
    _epoch,
    _epoch,
    FileSystemEntityType.notFound,
...
abstract class FileSystemEntity {
  ...
  static final RegExp _absoluteWindowsPathPattern = RegExp(
    r'^(?:\\\\|[a-zA-Z]:[/\\])',
  );
  ...
  static Uint8List _toNullTerminatedUtf8Array(Uint8List l) {
    if (l.isEmpty || (l.isNotEmpty && l.last != 0)) {
      final tmp = Uint8List(l.length + 1);
  ...
  static String _toStringFromUtf8Array(Uint8List l) {
    ...
    if (l.last == 0) {
      nonNullTerminated = Uint8List.view(
        l.buffer,
        l.offsetInBytes,
        l.length - 1,
      );
  ...
  static _throwIfError(Object result, String msg, [String? path]) {
    if (result is OSError) {
      throw FileSystemException(msg, path, result);
