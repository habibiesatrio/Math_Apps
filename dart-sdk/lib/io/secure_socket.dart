...
class _RawSecureSocket extends Stream<RawSocketEvent>
  ...
  final Completer<RawSecureSocket> _closeCompleter = Completer<RawSecureSocket>();
  ...
  Uint8List? read([int? length]) {
    ...
    if (_closedRead) {
      throw const SocketException("Reading from a closed socket");
    }
  ...
  int write(List<int> data, [int offset = 0, int? bytes]) {
    ...
    if (_closedWrite) {
      _controller.addError(const SocketException("Writing to a closed socket"));
      return 0;
    }
  ...
  void renegotiate({
    ...
    if (_status != connectedStatus) {
      throw const HandshakeException("Called renegotiate on a non-connected socket");
    }
  ...
  Future<_FilterStatus> _pushAllFilterStages() async {
    ...
    int newStart = start(writePlaintextId);
    ...
    newStart = start(readEncryptedId);
    if (newStart != buffer.start) {
      status.progress = true;
      if (buffer.free == 0) {
    ...
    int newEnd = end(writeEncryptedId);
...
class _ExternalBuffer {
  ...
  int writeFromSource(List<int>? Function(int requested) getData) {
    ...
    while (toWrite > 0) {
      ...
      if (inputData == null || inputData.isEmpty) break;
