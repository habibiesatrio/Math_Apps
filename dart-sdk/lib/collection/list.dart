...
abstract mixin class ListBase<E> implements List<E> {
  ...
  @override
  Iterable<E> where(bool Function(E element) test) => WhereIterable<E>(this, test);

  @override
  Iterable<T> whereType<T>() => WhereTypeIterable<T>(this);

  @override
  Iterable<T> map<T>(T Function(E element) f) => MappedListIterable<E, T>(this, f);
  ...
  List<E> toList({bool growable = true}) {
    if (isEmpty) return List<E>.empty(growable: growable);
    var first = this[0];
    var result = List<E>.filled(length, first, growable: growable);
    for (int i = 1; i < length; i++) {
      result[i] = this[i];
    }
    return result;
  ...
  void replaceRange(int start, int end, Iterable<E> newContents) {
    ...
    if (removeLength >= insertLength) {
      ...
      setRange(start, insertEnd, newContents);
      if (removeLength > insertLength) {
      ...
      if (insertEnd < oldLength) {
        setRange(insertEnd, oldLength, this, end);
      }
      setRange(start, insertEnd, newContents);
    }
  }
